...
import com.wzw.platform.dao.ArticleDao;
import com.wzw.platform.exception.ArticleException;
...
import com.wzw.platform.utils.ResponseStatus;
...
//        int userid = SecurityAuthenUtil.getId();
    ...
    public PageResult getArticlesByUserId(int userId, PageWrapper pageWrapper) {
        ...
        int size=articleDao.getArticleSizeByUserId(userId);
        return new PageResult<>(list,size);
    }
    ...
    public void updateArticle(ArticleUpdateRequest articleUpdateRequest) {
        ...
        int userId=1;
        ArticleEntity articleEntity = new ArticleEntity();
        articleEntity.setId(articleUpdateRequest.getId());
        articleEntity.setTitle(articleUpdateRequest.getTitle());
        articleEntity.setContent(articleUpdateRequest.getContent());
        articleEntity.setUserid(userId);
        String info = htmlText(articleEntity.getContent());
        if (info.length() > 100) {
            info = info.substring(0, 100);
